...
from fmri_mapping.embedding.ops import split_repetitions, split_betas_by_subject_index
...
DATA_TEMPLATE = "../scripts/steps/subject-{subject:02d}_betas_res.npy"
...
def process_subject(
    ...
    **kwargs,
    ...
    X_train_1, X_train_2, X_train_3 = split_betas_by_subject_index(betas=X, df_repetitions=df_reps_train, inpute="mean")
    Xs_train = [X_train_1, X_train_2, X_train_3]

    # Fit DCCA
    mcca = GCCA(n_components=n_components)

    mcca.fit(Xs_train)

    X_transformed = mcca.transform([X, X, X])
    X_transformed = (X_transformed[0] + X_transformed[1] + X_transformed[2]) / 3
...
def main():
    ...
    n_components = 128
    folder = Path("gcca_no-pca-results")
    folder.mkdir(exist_ok=True)

    results = []

    pbar = tqdm(total=8, desc="Total progress")

    for subject in range(1, 8 + 1):
        pbar.set_description(f"sub-{subject}")
        try:
            ...
            save_file = (
                folder
                / f"subject-{subject:02d}_gcca_layers_components-{n_components}.npz"
            )
            np.save(save_file, X)
            # Clear GPU memory
            del X
            ...
            if isinstance(e, KeyboardInterrupt):
                raise e
            send_message(f"Error processing subject {subject}: {e}")
    ...
    df.to_parquet(folder / f"evaluation_gcca_components-{n_components}.parquet")
