...
def main():
    ...
    for subject in range(1, 8 + 1):
        ...
        try:
            ...
            save_file = (
                folder
                / f"subject-{subject:02d}_mcca_layers_components-{n_components}.npz"
            )
            np.save(save_file, X)
            # Clear GPU memory
            del X
    ...
    df.to_parquet(folder / f"evaluation_mcca_components-{n_components}.parquet")
