...
DATA_TEMPLATE = "../scripts/steps/subject-{subject:02d}_betas_pca.npy"
MODELS_FOLDERS = "<path-to-models>/language_embeddings"
...
def process_subject_model_regression(
    subject: int, Y, test_stimulus: list[int], alpha: float = 1.0, **kwargs
):
    results = []
    ...
    return results, X_transformed


@alert(input=["model_path"])
def process_model(model_path: Path, test_stimulus: list[int], save_file: bool = True):
    pck = torch.load(model_path)
    features = pck["Z"].float()
    results = []

    for subject in trange(1, 8 + 1, leave=False, desc=f"Processing subject"):
        r, X = process_subject_model_regression(
            subject=subject,
            Y=features,
        ...
        results.extend(r)
        if save_file and X is not None:
            folder = Path("features_linear")
            folder.mkdir(exist_ok=True)
            filename = f"{model_path.stem}_subject-{subject:02d}_features.npy"
...
def main():
    ...
    for model in (pbar := tqdm(models, desc="Processing models")):
        ...
        r = process_model(model, test_stimulus=test_stimulus)
        results.extend(r)

    df = pd.DataFrame(results)
    filename = "language_results_models_regression-rel.parquet"
