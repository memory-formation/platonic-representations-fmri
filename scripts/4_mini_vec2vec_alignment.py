...
def parse_args():
    ...
    parser.add_argument(
        "--output_template",
        type=str,
        default="mini-vec2vec-alignment-sub-{subject_x:02d}-sub-{subject_y:02d}_{mean_rank:.2f}.pt",
        help="Template for output files. Should contain {subject_x} and {subject_y} placeholders.",
    )
    parser.add_argument(
        "--output_folder",
        type=str,
        default="mini_vec2vec_alignments",
    ...
    if args.subjects is not None:
        args.subjects = [(x, y) for x in args.subjects for y in args.subjects if y != x]
    elif args.subject_x is not None and args.subject_y is not None:
        args.subjects = [(args.subject_x, args.subject_y)]
    elif args.subjects is None:
        args.subjects = [(x, y) for x in range(1, 9) for y in range(1, 9) if y != x]
...
def main():
    ...
    for subject_x, subject_y in (pbar := tqdm(args.subjects)):
        pbar.set_description(f"Source=sub-{subject_x} Target=sub-{subject_y}")
        input_file_x = args.input_folder / args.input_template.format(subject=subject_x)
        input_file_y = args.input_folder / args.input_template.format(subject=subject_y)
        assert input_file_x.exists(), f"Input file {input_file_x} does not exist"
        assert input_file_y.exists(), f"Input file {input_file_y} does not exist"

        pck_source = torch.load(input_file_x)
        X_train = pck_source["Z_train"]
        X_test = pck_source["Z_test"]
        X_labels_test = pck_source["labels_test"]
        pck_target = torch.load(input_file_y)
        Y_train = pck_target["Z_train"]
        Y_test = pck_target["Z_test"]
        Y_labels_test = pck_target["labels_test"]

        min_n_test = min(len(X_test), len(Y_test))
        X_test = X_test[:min_n_test]
        Y_test = Y_test[:min_n_test]
        Y_labels_test = Y_labels_test[:min_n_test]
        X_labels_test = X_labels_test[:min_n_test]

        # Assert all labels equal
        assert torch.equal(X_labels_test, Y_labels_test), "Test labels do not match"
        ...
        pck["subject_x"] = subject_x
        pck["subject_y"] = subject_y
        ...
        output_file = args.output_folder / args.output_template.format(
            subject_x=subject_x, subject_y=subject_y, mean_rank=mean_rank
        )
        ...
        print(
            f"Finished alignment for sub-{subject_x} and sub-{subject_y}. "
