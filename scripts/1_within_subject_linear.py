...
def compute_linear_within_subject_pipeline(
    ...
    skip_mcca: bool = False,
    skip_mcca_distill: bool = False,
):
    ...
    if skip_mcca_distill:
        W_mcca, b_mcca = distill_mcca_projector(
        betas_pca[train_mask, :], mcca, lam_ridge=mcca_distill_reg, target="single_view"
    )
    else:
        W_mcca, b_mcca = distill_mcca_projector(
...
def parse_args():
    ...
    parser.add_argument(
        "--skip_residualization",
        action="store_true",
        help="Whether to skip confound residualization step",
    )
    parser.add_argument(
        "--skip_reliability",
        action="store_true",
        help="Whether to skip voxel reliability weighting step",
    )
    parser.add_argument(
        "--skip_mcca",
        action="store_true",
        help="Whether to skip MCCA step and use PCA components as final embedding",
    )
    parser.add_argument(
        "--skip_mcca_distill",
        action="store_true",
        help="Whether to skip MCCA distillation step and use MCCA components as final embedding",
    )
...
def main():
    ...
    skips = {
        "skip_residualization": args.skip_residualization,
        "skip_reliability": args.skip_reliability,
        "skip_mcca": args.skip_mcca,
        "skip_mcca_distill": args.skip_mcca_distill,
    }
